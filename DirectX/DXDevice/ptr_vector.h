...
#ifndef MG1_PTR_VECTOR_H
#define MG1_PTR_VECTOR_H
...
namespace mini {
    ...
    */
    ...
    public:
        ...
        template<typename InputIt, typename std::enable_if<std::is_default_constructible<deleter_type>::value &&
                                                           std::is_constructible<value_type, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
        ...
        template<typename InputIt, typename std::enable_if<std::is_default_constructible<deleter_type>::value &&
                                                           std::is_same<unique_ptr_type &&, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
        ptr_vector(InputIt first, InputIt last, const allocator_type &alloc = allocator_type{})
    ...
    private:
        ...
        template<typename InputIt, typename std::enable_if<std::is_default_constructible<_Dx>::value &&
                                                           std::is_same<std::unique_ptr<T, _Dx> &&, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
    ...
    void
    swap(typename ptr_vector<T, _Dx, _Alloc>::ptr_ref &x, typename ptr_vector<T, _Dx, _Alloc>::ptr_ref &y) noexcept {
        x.swap(y);
    }

    template<class T, class _Dx, class _Alloc>
    void
    swap(typename ptr_vector<T, _Dx, _Alloc>::iterator &x, typename ptr_vector<T, _Dx, _Alloc>::iterator &y) noexcept {
        x.swap(y);
    }
