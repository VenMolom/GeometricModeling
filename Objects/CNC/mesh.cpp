...
Mesh::Mesh(vector<VertexPositionColor> vertices, vector<Index> indices, XMFLOAT3 color, XMMATRIX modelMatrix,
           D3D11_PRIMITIVE_TOPOLOGY topology) : Renderable(topology), _color(color) {
...
std::vector<VertexPositionColor> Mesh::domeVerts(float radius, unsigned int stacks, unsigned int slices) {
    assert(stacks > 2 && slices > 1);
    auto n = stacks * slices + 1U;
    vector<VertexPositionColor> vertices(n);
    vertices[0].position = XMFLOAT3(0.0f, radius, 0.0f);
    vertices[0].color = XMFLOAT3(0.0f, 0.0f, 1.f);
    ...
    for (auto i = 0U; i < stacks; ++i, phi += dp)
    {
        ...
        for (auto j = 0U; j < slices; ++j, thau += dt)
        {
            ...
            vertices[k].position = XMFLOAT3(stackR * cost, stackR * sint, stackY);
